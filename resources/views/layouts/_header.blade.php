<nav class="navbar navbar-expand-lg navbar-light bg-light sticky-top navbar-static-top">
    <div class="container">
        <a href="{{ url('/') }}" class="navbar-brand">
            <img src="{{ env('APP_URL') }}/capinno_logo.png" alt="" width="150px" height="25px">
        </a>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <ul class="navbar-nav navbar-right">
                ...
                @else
                <a href="@if($user_sign_id = getUserSignId()){{ route('user_signs.show', ['user_sign' => $user_sign_id]) }}@else{{ route('user_signs.create') }}@endif" class="nav-item btn btn-primary mr-3">报名参赛</a>
                <li class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle name" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="{{ Auth::user()->name }}">
                        <img src="https://iocaffcdn.phphub.org/uploads/images/201709/20/1/PtDKbASVcz.png?imageView2/1/w/60/h/60" class="img-responsive img-circle" width="30px" height="30px" style="display: none;">
                        <span>{{ Auth::user()->name  }}</span>
                    </a>
                    <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                        @if($user_sign_id)
                        <a href="{{ route('user_signs.show', ['user_sign' => $user_sign_id]) }}" class="nav-item dropdown-item">我的赛事</a>
                        @endif
